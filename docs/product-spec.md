...
The Organizer and Team Representatives can browse a read-only Player directory while Live or Paused. It includes every Tier, including unopened Tiers, and supports combined Player-name search and Tier/status/Team filters, sortable columns, resettable filters, and configurable pagination. TanStack Table manages filtering, sorting, and pagination; shadcn Select provides the dropdown controls. Each Player shows their current status, acquired Team, and Sale price when assigned. Waiting Players, active bidding, the Unsold Pool, Final Unsold, Forced Assignments, and preassigned Player Representatives remain distinct. Counts distinguish Sales, representatives, and remaining Players. The directory updates with the authorized live snapshot, preserves browsing choices across updates, flags stale state during reconnection, and never includes phone numbers or custom fields. Mobile cards use the same filtered and sorted rows. Dropdown keyboard navigation never triggers Auction shortcuts.
