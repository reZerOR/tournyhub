...
export function LiveOrganizerTools({
  ...
  const [activeTab, setActiveTab] = useState<
    "tiers" | "corrections" | "direct-sale" | "rejections" | "shortcuts"
  >("tiers");

  return (
    <div className="overflow-hidden rounded-2xl border border-border/70 bg-card/60 shadow-sm backdrop-blur-md transition-all">
      {/* Drawer Toggle Header */}
      <button
        className="flex w-full items-center justify-between gap-3 px-4 py-3.5 text-left transition-colors hover:bg-muted/20 sm:px-6"
      ...
      {/* Expandable Content */}
      {isOpen && (
        <div className="flex flex-col gap-5 border-t border-border/60 p-4 sm:p-6">
          {/* Tab Selection */}
            ...
            <div className="flex flex-col gap-4">
                ...
                {tiers.map((tier) => (
                  ...
                  >
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span
                        className={
                          tier.isActive ? "text-neon" : "text-foreground"
                        }
                      >
              ...
              {unsoldRound && (
                <p className="text-xs text-muted-foreground">
                  Unsold Round {unsoldRound.sequence}:{" "}
                  {unsoldRound.eligibleCount} eligible,{" "}
          ...
          {activeTab === "corrections" && lifecycle === "paused" && (
            <div className="flex flex-col gap-4">
              <Field className="max-w-xl">
                <FieldLabel htmlFor="tools-correction-reason">
                  Correction Reason
                </FieldLabel>
                <Input
              ...
              <div className="flex flex-wrap items-end gap-3 border-t border-border/40 pt-3">
                <Field className="max-w-md min-w-64 flex-1">
                  <FieldLabel htmlFor="tools-reverse-sale">
                    Sale to Reverse
                  </FieldLabel>
                  <Select
                  ...
                  >
                    <SelectTrigger
                      ...
                      id="tools-reverse-sale"
                    >
                      <SelectValue placeholder="Choose a completed Sale">
                        {(val: string | null) => {
                          const sale = openSales.find((s) => s.saleId === val);
                          return sale
                            ? `${sale.playerDisplayName} · ${sale.amount} cr${sale.source === "forced" ? " (Forced)" : sale.source === "direct" ? " (Direct)" : ""}`
                            : (val ?? undefined);
                        }}
                      </SelectValue>
                    </SelectTrigger>
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-wrap items-end gap-3">
                ...
                <Field className="min-w-48 flex-1">
                  <FieldLabel htmlFor="tools-direct-sale-team">Team</FieldLabel>
                  <Select
                    items={teams.map((t) => ({
                      label: t.name ?? t.id,
                      value: t.id,
          ...
          {activeTab === "rejections" && (
            <div className="flex flex-col gap-2">
              <span className="text-xs text-muted-foreground">
                Recent rejected bids (visible only to the Organizer and
                submitting team):
              </span>
              {rejections.length > 0 ? (
